...
export class SalesComponent implements OnInit {
  ...
  count: number;
  selection = new SelectionModel<salesList>(true, []);
  ...
  salesData(){
    let loadSublocationList = this.salesService.salesGetAll().subscribe(data => {
      ...
      let search = [
        { name: "Sales Orgainsation Code", value: 'salesOrgCode' },
        { name: "Sales Organisation Name", value: 'salesOrgName' },
    ...
    // let loadSearch = this.salesService.search1().subscribe(data => {
  ...
  projectView() {
    if (this.selection.selected.length === 1) {
      this.componentLoaderService.display(true);
      this.router.navigate(['/sales/sales-view']);
  ...
  projectModify() {
    if (this.selection.selected.length === 1) {
      ...
      dialogRef.afterClosed().subscribe(data => {
        if (data) {
          this.router.navigate(['/sales/sales-modify']);
  ...
  removeSelectedRows() {
    if (this.selection.selected.length === 1) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.salesService.delete(id).subscribe(data => {
  ...
  onSubmitSearch(val) {
    ...
    finalSearchData['accessToken']  = localStorage.getItem("kiosk_access_token");
     this.salesService.search(finalSearchData).subscribe(data => {
