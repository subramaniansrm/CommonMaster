...
export class DepartmentComponent implements OnInit {
  ...
  onLoadDepartmentRoleList() {
    let loadUserRoleList = this.deparmentService.userRoleList().subscribe(data => {
      ...
      this.dataSource = []
      if (loadUserRoleListGetData.succesObject.departmentList !== null) {
      ...
      let search = [
        { Name: 'Choose Field' },
        { Name: "User Department Name", Value: 'departmentName' },
        { Name: "User Location", Value: 'locationName' },
        { Name: "User Sub-Location", Value: 'sublocationName' },
  ...
  removeSelectedRows() {
    if (this.selection.selected.length === 1) {
      ...
      let userRoleId = [this.selection.selected[0].departmentId];
      ...
      for (let i = 0; i < this.selection.selected.length; i++) {
        userRoleId.push(this.selection.selected[i].departmentId);
      }
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.deparmentService.deleteUserRoleList(userRoleId).subscribe(data => {
  ...
  projectModify() {
    if (this.selection.selected.length === 1) {
      ...
      dialogRef.afterClosed().subscribe(data => {
        if (data) {
          let locId = this.selection.selected[0]['locationId'];
          console.log("selection" , this.selection.selected[0]);

          this.router.navigate(['/department/department-modify']);
         
          let rowId = String(this.selection.selected[0]['departmentId']);
          console.log(rowId , " sdf");
