...
export class CustomerComponent implements OnInit {
  ...
  highlight(element: customerList) {
  ...
  customerData(){
    let loadSublocationList = this.customerService.customerGetAll().subscribe(data => {
      ...
      let search = [
        ...
        { name: "Sales Oraganisation Name", value: 'salesOrgName' },
        { name: "Distribution Name", value: 'distributionName' },
        { name: "Plant Name", value: 'plantName' },
        { name: "Area Nagar", value: 'areaNagar' },
        { name: "Parent Code", value: 'parentCode' },
        { name: "Parent Name", value: 'parentName' },
    ...
    // let loadSearch = this.customerService.search1().subscribe(data => {
  ...
  projectView() {
    if (this.selection.selected.length === 1) {
      this.componentLoaderService.display(true);
      this.router.navigate(['/customer/customer-view']);
  ...
  projectModify() {
    if (this.selection.selected.length === 1) {
      ...
      dialogRef.afterClosed().subscribe(data => {
        if (data) {
          this.router.navigate(['/customer/customer-modify']);
  ...
  onSubmitSearch(val) {
    ...
    finalSearchData['accessToken']  = localStorage.getItem("kiosk_access_token");
     this.customerService.search(finalSearchData).subscribe(data => {
