...
import { ViewUserSalesService } from '../view-user-sales/view-user-sales.service';
...
export class UpdateComponent implements OnInit {
  ...
  salesFn: any = [];
  getSalePlantValue(ev, data,name) {
    let obj = {
      salesOrgId: data,
      name : name,
    };
  ...
  getPlantValue(eve, id, item) {
    ...
    console.log(this.salesFn, item);
console.log("lengt",this.salesFn.length)

    if(this.salesFn.length >1){
  ...
  baseFilterCheck(eve, id, item) {
    ...
    if (eve.checked === true) {
      ...
      for (let i = 0; i < userScreenFieldMappingVOList.length; i++) {
        if (userScreenFieldMappingVOList[i]["screenFieldId"] === id) {
          userScreenFieldMappingVOList[i]["activeFlag"] = false;
        }
      }
    }
  }
  ...
  baseFilterCheckFunction(eve, id, item) {
    console.log(item)
    let len = this.newScrFn.length - 1;
    let userScreenFunctionMappingVOList = this.newScrFn[len].userScreenFunctionMappingVOList;
    if (eve.checked === true) {
      for (let i = 0; i < userScreenFunctionMappingVOList.length; i++) {
        if (userScreenFunctionMappingVOList[i]["screeFuncionId"] === id) {
          userScreenFunctionMappingVOList[i]["activeFlag"] = true;
        }
      }
    } else {
      for (let i = 0; i < userScreenFunctionMappingVOList.length; i++) {
        if (userScreenFunctionMappingVOList[i]["screeFuncionId"] === id) {
          userScreenFunctionMappingVOList[i]["activeFlag"] = false;
        }
      }
    }
  }
  ...
  allNonTradesFunction(event) {
    console.log(event);
    if (event.checked === true) {
      this.newScrFn[this.newScrFn.length - 1].userScreenFunctionMappingVOList = [];
      for (let i = 0; i < this.screenFucntionList.length; i++) {
        let eve = { checked: true };
        this.selectFunctions(eve, this.screenFucntionList[i].screenFunctionId, this.screenFucntionList[i]);
      }
    } else {
      this.newScrFn[this.newScrFn.length - 1].userScreenFunctionMappingVOList = [];
      for (let i = 0; i < this.screenFucntionList.length; i++) {
